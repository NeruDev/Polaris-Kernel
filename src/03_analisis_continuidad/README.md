...
- **`limite_concepto.svg`**: Muestra la aproximación asintótica. Ajustar las flechas de acercamiento (límite por izquierda y derecha) usando las coordenadas de `line(..., mark: (end: ">"))`.
- **`derivada_concepto.svg`**: Muestra la pendiente de la recta tangente. Si se altera la función base, asegurarse de recalcular y rotar la línea recta tangente en el punto de interés.
...
- **`campo_vectorial.svg` / `edo_campo_direcciones.svg`**: 
  ...
  - **Parámetros:** La longitud de los vectores (normalización) y la densidad de la rejilla. Ajustar el paso del rango si el campo se ve demasiado sobrecargado.
