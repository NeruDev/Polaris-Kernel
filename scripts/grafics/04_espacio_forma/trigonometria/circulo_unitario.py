...
def generate():
    ...
    ax.plot([0, x], [0, 0], color=colors["accent"], lw=3, label=r"$\cos(\theta)$")
    ax.plot([x, x], [0, y], color=colors["secondary"], lw=3, label=r"$\sin(\theta)$")
    arc = patches.Arc((0, 0), 0.4, 0.4, theta1=0, theta2=45, color=colors["primary"], lw=1)
    ax.add_patch(arc)
    ax.text(0.25, 0.1, r"$\theta$", color=colors["primary"], fontsize=12)
    ax.set_xlim(-1.2, 1.2)
    ax.set_ylim(-1.2, 1.2)
    ax.set_aspect("equal")
