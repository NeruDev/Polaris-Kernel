...
def draw_angle(ax, center, angle, label, color):
    ax.plot([center[0], center[0] + 1.5], [center[1], center[1]], color=color, lw=2)
    rad = np.radians(angle)
    ax.plot(
        [center[0], center[0] + 1.5 * np.cos(rad)],
        [center[1], center[1] + 1.5 * np.sin(rad)],
        color=color,
        lw=2,
    )
    arc = patches.Arc(center, 0.8, 0.8, theta1=0, theta2=angle, color=color, lw=1.5, ls="--")
    ax.add_patch(arc)
    ax.text(center[0] + 0.5, center[1] + 0.8, label, ha="center", fontsize=10, fontweight="bold")
