...
def generate():
    ...
    # Punto de limite
    a, l_val = 1.5, 1.5**2
    ax.scatter(a, l_val, color=colors["danger"], zorder=5)

    # Lineas de tendencia
    ax.hlines(l_val, 0, a, colors=colors["text"], ls="--", alpha=0.3)
    ax.vlines(a, 0, l_val, colors=colors["text"], ls="--", alpha=0.3)
    ...
    ax.set_title(r"Concepto de Limite: $\lim_{x \to a} f(x) = L$")
    ax.set_xlabel("x")
    ax.set_ylabel("f(x)")
    ax.legend()
