...
def generate():
    ...
    a_val, b_val = 1, 3
    ix = np.linspace(a_val, b_val)
    iy = f(ix)
    ax.fill_between(ix, iy, color=colors["secondary"], alpha=0.3, label="Area definida")

    # Rectangulos de Riemann (ejemplo con 6)
    n_rects = 6
    dx = (b_val - a_val) / n_rects
    x_rects = np.linspace(a_val, b_val - dx, n_rects)
    ax.bar(
        x_rects,
        f(x_rects),
        width=dx,
