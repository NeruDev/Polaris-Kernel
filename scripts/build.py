...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--generate-assets",
        action="store_true",
        help="Ejecutar scripts manuales para generar imágenes SVG (Restringido por defecto para Quarto/Typst)",
    )
...
def validate_project(config: BuildConfig, file_manager: FileManager, collector: ErrorCollector):
    ...
    for md_path in [p for ext in ("*.md", "*.qmd") for p in config.paths.src_dir.rglob(ext)]:
        try:
            ...
            # Validación de Saltos de Línea Semánticos (Semantic Line Breaks)
            from utils.markdown import check_semantic_line_breaks

            semantic_warnings = check_semantic_line_breaks(content)
            for sem_warn in semantic_warnings:
                collector.add_message(
                    "semantic_style", f"[{md_path.name}] {sem_warn}", critical=config.strict
                )
...
def run_site(config: BuildConfig, file_manager: FileManager, collector: ErrorCollector):
    ...
    try:
        ...
        result = subprocess.run(
            "quarto render", capture_output=True, text=True, cwd=str(PROJECT_ROOT), shell=True
        )
        if result.returncode != 0:
        ...
        collector.add_message(
            "rendering", "Quarto CLI no está instalado o no se encuentra en el PATH.", critical=True
        )
    except Exception as e:
...
def run_build():
    ...
    config = BuildConfig(paths=paths, verbose=args.verbose, strict=args.strict)
    ...
    # Auditamos enlaces del sitio generado en Quarto
    if not collector.has_critical_errors:
        log_info("Auditando enlaces internos y recursos en el sitio generado...")
        from utils.links import detect_broken_internal_links

        broken = detect_broken_internal_links(config.paths.project_root / "site")
        for file, link in broken:
            collector.add_message(
                "broken_links", f"En {file}: Enlace/recurso roto '{link}'", critical=config.strict
            )

    if collector.has_critical_errors:
        log_error("\nBuild finalizado con errores críticos:")
        for line in collector.format_summary():
            print(line)
        sys.exit(1)
