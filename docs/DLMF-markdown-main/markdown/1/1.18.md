...
$L^{2}\left(X,\,\mathrm{d}x\right)$ : Lebesgue–Stieltjes measurable, square integrable, complex-valued functions
...
<a id="E54"></a>
$$
\lim_{\epsilon\to 0{+}}\int_{X}\frac{f(y)}{x\pm\mathrm{i}\epsilon-y}\,\mathrm{d}y=P\fint_{X}\frac{f(y)}{x-y}\,\mathrm{d}y\mp\mathrm{i}\pi f(x). \tag{1.18.54}
$$
...
$\,\mathrm{d}x$: differential of $x$ , $\int$: integral and $\left|x\right|$: absolute value of $x$
