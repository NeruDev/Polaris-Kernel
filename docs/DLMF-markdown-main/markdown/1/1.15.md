...
$\pi$: the ratio of the circumference of a circle to its diameter , $\,\mathrm{d}x$: differential of $x$ , $\mathrm{e}$: base of natural logarithm , $\mathrm{i}$: imaginary unit , $\int$: integral , $F(x)$: Fourier transform of $f(t)$ , $\sigma_{R}(\theta)$: function and $\left|x\right|$: absolute value of $x$
...
$\,\mathrm{d}x$: differential of $x$ , $\int$: integral , $K_{R}(s)$: Fejér kernel and $\sigma_{R}(\theta)$: function
