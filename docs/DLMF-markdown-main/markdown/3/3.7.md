...
$!$: factorial (as in $n!$) , $g_{s}(z)$: coefficient , $A_{jk}(\tau,z)$: matrix entry and $f_{s}(z)$: coefficient
...
$!$: factorial (as in $n!$) , $b_{j}(\tau,z)$: vector and $h(z)$: function
...
$O\left(x\right)$: order not exceeding , $w(z)$: function and $h(z)$: function
