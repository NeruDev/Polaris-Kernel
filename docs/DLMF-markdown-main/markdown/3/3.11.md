...
$J_{\nu}\left(z\right)$: Bessel function of the first kind and $R_{k,\ell}(x)$: rational approximation
...
<a id="E26"></a>
$$
F(s)=\mathscr{L}\mspace{-3.0mu}f\mspace{3.0mu}\left(s\right)=\int_{0}^{\infty}e^{-st}f(t)\,\mathrm{d}t \tag{3.11.26}
$$
...
Here $x_{j}$ , $j=1,2,\dots,J$ , is a given set of distinct real points and $J\geq n+1$ . From the equations $\frac{\partial S}{\partial a_{k}}=0$ , $k=0,1,\dots,n$ , we derive the *normal equations*
