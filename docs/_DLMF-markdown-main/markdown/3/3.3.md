...
<a id="E1"></a>
$$
P_{n}(z)=\sum_{k=0}^{n}\ell_{k}(z)f_{k}=\omega_{n+1}(z)\sum_{k=0}^{n}\frac{\widetilde{\omega}_{k}}{z-z_{k}}f_{k}=\frac{\sum_{k=0}^{n}\frac{\widetilde{\omega}_{k}f_{k}}{z-z_{k}}}{\sum_{k=0}^{n}\frac{\widetilde{\omega}_{k}}{z-z_{k}}}, \tag{3.3.1}
$$
...
$\delta_{j,k}$: Kronecker delta and $z_{k}$: nodes of function
...
$R_{n,t}(x)$: remainder and $f_{t}$: function values
...
$\left[z_{0},z_{1},\dots,z_{n}\right]f$ : divided difference

**Symbols:**

$f(z)$: function , $f_{t}$: function values and $z_{k}$: nodes of function
...
<a id="E35"></a>
$$
\left[z_{0},z_{1},\dots,z_{n}\right]f=\sum_{k=0}^{n}\left(\frac{f(z_{k})}{\prod_{\begin{subarray}{c}0\leq j\leq n\\
